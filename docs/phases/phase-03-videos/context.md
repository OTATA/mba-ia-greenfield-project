...
sources_mtime:
  ...
  docs/phases/phase-02-auth-frontend/context.md: "2026-09-11T10:26:34+00:00"
  .claude/skills/testing-guide-nestjs-project/SKILL.md: "2026-09-11T10:26:34+00:00"
---
...
Notes carried from the guide that bear directly on this phase's new artifact types:

- "Module with configured imports" explicitly names `BullModule.registerQueue()` alongside `TypeOrmModule.forFeature()` and `JwtModule.register()` as requiring a compilation test.
- "Service with side-effect dep (email, storage)" resolves to **Integration with a real capture service** — the same posture the project already applies to Mailpit.
- Race conditions are listed under "Worth testing", naming **concurrent video uploads** specifically.
- Detailed external-system strategies (DB, storage, queue, email) live in the guide's `references/external-systems.md`.

### next-frontend

_Deferred subproject for this phase — Phase 03 delivers no frontend artifact, so no layer requirements apply here. See `## Inherited Deferred Capabilities`._
