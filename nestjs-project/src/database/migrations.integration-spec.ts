...
import { CreateVideos1789135016153 } from './migrations/1789135016153-CreateVideos';
...
describe('Database migrations (integration)', () => {
  ...
  it('should drop the videos status enum type on revert', async () => {
    // Enum types survive DROP TABLE, so down() must remove them explicitly —
    // otherwise a re-run fails on CREATE TYPE with "type already exists".
    await dataSource.undoLastMigration();
